...
export const ProductCard: React.FC<Props> = ({ product }) => {
  ...
  const handleLikeClick = async (like: Product) => {
    ...
    if (!likedProduct) {
      try {
        await dispatch(addNewToLikes(userId, like));
      ...
      await dispatch(
        deleteItemFromLikes(like.itemId, userId)
      )
    }
  };

  const handleAddToCartClick = async (productToCart: Product) => {
    if (!isAuthenticated || !user) {
      navigate('/login');
      return;
    }
    ...
    if (!addedToCartProduct) {
      try {
        await dispatch(addNewToCart(userId, productToCart, 1));
      ...
      await dispatch(
        deleteItemFromCart(productToCart.itemId, `${userId}`, productToCart.itemId)
      )
    }
  };
