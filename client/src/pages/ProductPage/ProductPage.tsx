...
import { addNewToCart, deleteItemFromCart } from '../../api/addToCart';
import { addNewToLikes, deleteItemFromLikes } from '../../api/addToLike';
...
export const ProductPage = () => {
  ...
  const handleLikeClick = async (fullProduct: FullProduct) => {
    ...
    if (!likedProduct) {
      ...
      try {
        await dispatch(addNewToLikes(userId, likeProductClick));
      ...
      await dispatch(
        deleteItemFromLikes(fullProduct.id, userId)
      )
    }
  };

  const handleAddToCartClick = async (fullProduct: FullProduct) => {
    if (!isAuthenticated || !user) {
      navigate('/login');
      return;
    }
    ...
    if (!addedToCartProduct) {
      ...
      try {
        await dispatch(addNewToCart(userId, productCartClick, 1));
      ...
      await dispatch(
        deleteItemFromCart(fullProduct.id, `${userId}`, fullProduct.id)
      )
    }
  };
              ...
              <div className="settings-product">
                ...
                <div className="settings-product__purchase">
                  ...
                  <div className="purchase__buttons">
                    ...
                    >
                      {!addedToCartProduct ? 'Add to cart' : 'Added to cart'}
                    </button>
